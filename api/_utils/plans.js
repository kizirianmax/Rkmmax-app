...
const tierOf   = (k) => k.startsWith("basic") ? "basic"
                 : k.startsWith("intermediate") ? "intermediate"
                 : "premium";
...
export function getPlanByKey(key) {
  const plan = meta[key];
  if (!plan) return null;
  return { key, ...plan, tier: tierOf(key), region: regionOf(key), price_id: PRICE_IDS[key] || null };
}
...
export function listPlansByRegion(region = "BR") {
  ...
  return Object.keys(meta).filter(k => k.endsWith(suffix)).map(getPlanByKey).filter(Boolean);
}
