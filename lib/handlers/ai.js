 ...
 * - GROQ REASONING (GPT-OSS 120B) = Raciocínio profundo, análises complexas, código
 * - GROQ STANDARD (Llama 3.3 70B) = Uso geral, conversas normais
 * - GROQ SPEED (Llama 3.1 8B) = Velocidade, respostas rápidas
...
const GROQ_MODELS = {
  ...
  // 120B parâmetros = Melhor modelo disponível para tarefas complexas
  reasoning: 'openai/gpt-oss-120b',
  
  // ⚡ Uso geral (conversas normais, perguntas médias)
  // 70B parâmetros = Equilíbrio entre velocidade e qualidade
  standard: 'llama-3.3-70b-versatile',
  
  // 🚀 Velocidade (respostas rápidas, perguntas simples)
  // 8B parâmetros = Modelo mais rápido disponível
...
function selectGroqModel(messages) {
  ...
  // Decisão
  if (complexScore >= 2) return 'reasoning';  // GPT-OSS 120B
  if (simpleScore >= 2) return 'speed';       // Llama 3.1 8B
