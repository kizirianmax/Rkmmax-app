...
/**
 * Validate Claude API key configuration
 * @throws {Error} if API key is not configured or invalid
 */
function validateClaudeApiKey() {
  if (!CLAUDE_CONFIG.apiKey) {
    throw new Error(
      'ANTHROPIC_API_KEY is required. Please set it in your environment variables. ' +
      'Visit https://console.anthropic.com/settings/keys to get your API key.'
    );
  }
  
  // Validate API key format (must start with sk-ant-)
  if (!CLAUDE_CONFIG.apiKey.startsWith('sk-ant-')) {
    throw new Error(
      'Invalid ANTHROPIC_API_KEY format. API key must start with "sk-ant-". ' +
      'Please check your API key at https://console.anthropic.com/settings/keys'
    );
  }
}

// Validate API key on module load with a warning (not fatal)
try {
  validateClaudeApiKey();
} catch (error) {
  console.warn('⚠️ ANTHROPIC_API_KEY not configured. Claude integration will not work.');
  console.warn('📝 Set ANTHROPIC_API_KEY in your environment variables.');
  console.warn('📚 See .env.template for configuration instructions.');
}
...
class ClaudeClient {
  ...
  async processarTexto(prompt, config = {}) {
    try {
      console.log('🔑 Claude SDK Request:', {
        hasApiKey: !!CLAUDE_CONFIG.apiKey,
        apiKeyPrefix: CLAUDE_CONFIG.apiKey?.substring(0, 10) + '...',
        model: config.model || CLAUDE_CONFIG.model,
        maxTokens: config.maxTokens || CLAUDE_CONFIG.maxTokens
      });
  ...
  async processarComImagem(prompt, imagemBase64, mediaType = 'image/jpeg') {
    try {
      console.log('🔑 Claude SDK Image Request:', {
        hasApiKey: !!CLAUDE_CONFIG.apiKey,
        apiKeyPrefix: CLAUDE_CONFIG.apiKey?.substring(0, 10) + '...',
      ...
      console.error('❌ Erro processamento imagem:', {
        message: error.message,
        type: error.type,
        status: error.status
      });
  ...
  async processar(requisicao, opcoes = {}) {
    try {
      ...
      if (analise && analise.tipo_tarefa) {
        ...
        console.log('⚠️ Análise falhou, usando configuração padrão');
      }
