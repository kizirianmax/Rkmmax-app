...
export default function AdvancedDashboard() {
  const [metrics, setMetrics] = useState({
    ...
    apis: {
      openai: { calls: 1250, cost: 12.50, status: 'active' },
      anthropic: { calls: 850, cost: 8.50, status: 'active' },
  ...
  const apiCallsData = [
    { name: 'OpenAI', calls: 1250, cost: 12.50 },
    { name: 'Anthropic', calls: 850, cost: 8.50 },
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {Object.entries(metrics.apis).map(([provider, data]) => (
              <div key={provider} className="bg-slate-700 rounded-lg p-4">
                ...
                <div className={`text-xs mt-2 ${data.status === 'active' ? 'text-green-400' : 'text-red-400'}`}>
